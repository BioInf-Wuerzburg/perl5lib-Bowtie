...
# $Id: Bowtie.pm 55 2013-05-15 11:41:39Z s187512 $
...
SHRiMP (2.2.0) bowtie interface.

=head1 SYNOPSIS

  use Bowtie;
  
  my $bowtie = Bowtie->new(
    ref => 'genome.fa'
    reads => 'reads.fa'
  );
...
=item [Docu] Added some synopsis and adjusted parameter docu off C<< $bowtie->new >>
...
Added C<< $bowtie->status >> method to retrieve the current status message.

=item 0.02

Removed IPC::Open3. Does not work with bowtie, do know why. STDOUT
...
our $V = Verbose->new(
	format => "[{TIME_SHORT}] [Bowtie] {MESSAGE}\n"
);
...
=head2 new
  ...
  out => 'FILE'    # file argument to write data to. If specified, 
                   ...
                   #  the complete result is provided, after the run has finished.
  log => 'FILE'    # A file to write the log to, defaults to 'bowtieXXXXX.log', 
                   #  with X being random numbers
  verbose => BOOL  # Default TRUE
  timeout => INT   # Number of seconds before canceling run by timeout
  bin => 'bowtie'
                   # use explicit path, in case bowtie cannot be found in
                   #  the path, use bowtie -C for color space
  ref => [FILE, FILE,...]
...
Start bowtie run with parameters provided to new method. Returns bowtie object.

  my $bowtie = $bowtie->run;
...
Public Method. Waits for the finishing/canceling of a 
 started bowtie run. Checks for errors, removes tempfiles.
...
sub finish{
	my ($self) = @_; 
	unless (ref $self || ref $self !~ /^Bowtie/ ){
		die "Bowtie not initialized!\n";
	}
	
	# to make sure, bowtie is finished, read its STDOUT until eof
	unless($self->{out}){
		my $tmp;
...
Public Method. Cancels a running bowtie run based on a passed query process id 
or the internally stored process id of the bowtie object;

  my $bowtie->cancel(<message>);
  
  Bowtie->cancel(pid, <message>);
...
#		$pid." doesnt exist -> probably already finished\n");
	...
	$V->verbose($msg || "Bowtie run canceled");
}
...
Get the output handle to read the output while generated by bowtie
...
Get the log handle to read the log generated by bowtie
...
Get the status message ("running", "done", "canceled") of the bowtie run.
...
sub _build_command{
	...
	# [ -S/--save <filename> ]
	#    With this parameter,  bowtie projects and indexes  the genome, and saves it
...
sub _run_pipe_open {
	...
	if($self->{out}){
		...
		# get the entry for the all running bowtie processes
